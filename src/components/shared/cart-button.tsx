...
export const CartButton: FC<Props> = ({ className }) => {
  const { cost, loading, cartProduct } = useCartStore();

  return (
    <CartDrawer>
      <Button loading={loading} className={cn('group relative', className)} size={'lg'}>
        ...
        <span className="flex items-center gap-1 transition duration-300 group-hover:opacity-0">
          ...
          <span className="font-bold">{cartProduct.length}</span>
        </span>
        <ArrowRight
          size={20}
